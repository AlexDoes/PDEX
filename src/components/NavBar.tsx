...
export default function NavBar() {
  ...
  return (
    <>
      {show && (
        <div
          className="flex   w-full gap-2 h-16 bg-[#c1e1c1] max-h-16 sm:justify-between border-b-2 border-[#ffcd04]
          "
        >
          <Link href={"/"} className="inherit">
            <div className="flex flex-row h-full md:ml-[4vw] items-center  transition-all ease-in-out duration-500">
              <img
                ...
                className="h-full"
              />
              <div className="sm:text-[50px] text-3xl border border-red-500 font-serif text-[#efe6c1] text-shadow-md">
                BAX
              </div>
            </div>
          </Link>
