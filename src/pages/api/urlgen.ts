...
const urls = [  'https://perenual.com/api/species-list?page=1&key=sk-HpTB63eaa80ae182745',
...
const fetchAllPagesData = async (urls) => {
  const responses = await Promise.all(urls.map(fetchPageData));
  return responses;
};
...
fetchAllPagesData(urls)
  .then((data) => {
    ...
    data.forEach((pageData, index) => {
      fs.writeFile(`./src/pages/api/data/page${index + 1}.json`, `${JSON.stringify(pageData)}`, (err) => {
  if (err)
    console.log(err);
  else {
